...
Meteor.methods({
    'saveFile': function(buffer){
        console.log("Where the fuck is the server");
        console.log(buffer);
        source = fs.createReadStream(buffer);
        var file = fs.createWriteStream('../public/imgs/myimage.jpg');
        source.pipe(file);
        //file.write(buffer);
        //file.end();
        console.log('Finish uploading, time taken: ' + Date.now() - start);
        console.log("we now trigger the server");
    }
});
}
